...
# Script adapted by Cristina Martínez Rendón
# 27-11-2024
...
# 0. Chosen model
# Following Odriozola et al. (2021)
# The data sets are characterized by a high frequency of zeros, indicating many species absences, which necessitated the use of a **hurdle model**. This modeling approach has two components: one for presence-absence and another for abundance conditioned on presence.
...
#  We included in the analyses only those OTUs with a prevalence of at least 10% among sampling units, consistent with the approach used for network analyses in FlashWeave. This threshold was chosen because data on rare species lack sufficient informativeness to support the fitting of reliable species-specific models (Ovaskainen & Abrego, 2020).
...
### 4. Add trait data (TrData matrix)
  ...
  #I save the taxa order for use it to generate the species networks figures
...
### 6. MODEL SETUP!
  ...
  # Random effects, to model variation at each levels that won't be taken as fixed effects, but rather as sources of variability. This is commonly done in hierarchical or mixed-effects models.
  # The rL.xxx objects specify random effects. The setPriors() function determines the number of latent factors (nfMin and nfMax) that can explain random variation. This is where spatial or sample-level variability is captured, critical for community ecology models.
  ...
  ### Run HMSC for presence-absence data with 140 iterations. This also works.
  t0 <- Sys.time()
