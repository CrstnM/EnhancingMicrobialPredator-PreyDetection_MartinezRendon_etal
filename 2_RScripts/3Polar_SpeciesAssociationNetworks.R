...
### Species association networks with HMSC models
...
### 1. Read taxa order for use it to generate the species networks figures
desired_order <- read.csv(
  file = file.path(dataDir, "desired_order.csv"))$x


# Loop through models for association computations. This loop computes the complete netowrks.
...
### 2.  Trait-based filtering to exclude bacterivores, for a second version of the species networks
...
### 3.  Trait-based filtering to exclude algivores and omnivores, a third version of the species networks
...
VP <- computeVariancePartitioning(sampled_model)
